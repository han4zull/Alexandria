...
<head>
  <meta charset="UTF-8">
  <title>Dashboard Admin | Alexandria Library</title>
  ...
  <main class="flex-1 bg-[#faf6ee]/80 rounded-3xl p-8 shadow-xl max-w-[1400px] mx-auto">
    ...
    <div class="welcome-banner mb-10 p-10 rounded-3xl text-white shadow-2xl">
      <h1 class="judul text-3xl mb-2">
        Selamat datang,
        {{ $admin->jenis_kelamin === 'L' ? 'Pak' : 'Bu' }}
        <span class="italic">{{ $admin->nama ?? 'Admin' }}</span>
      </h1>
      <p class="text-sm opacity-90 max-w-xl">
        Dashboard Alexandria Library — tempat mengatur publikasi buku, peminjaman, dan akun dengan elegan & aman.
      </p>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-12">
      ...
      <div class="p-6 rounded-2xl bg-white shadow-lg">
        <p class="text-xs text-[#6b5a4a]">Buku Menunggu Publikasi</p>
        <h3 class="text-3xl font-semibold">{{ $stats['pending'] }}</h3>
        <p class="text-xs text-[#9a8b7a] mt-2">Menunggu konfirmasi</p>
      </div>

      <div class="p-6 rounded-2xl bg-white shadow-lg">
        <p class="text-xs text-[#6b5a4a]">Peminjaman Aktif</p>
        <h3 class="text-3xl font-semibold">{{ $stats['dipinjam'] }}</h3>
        <p class="text-xs text-[#9a8b7a] mt-2">Sedang dipinjam</p>
      </div>

      <div class="p-6 rounded-2xl bg-white shadow-lg">
        <p class="text-xs text-[#6b5a4a]">Total Petugas</p>
        <h3 class="text-3xl font-semibold">{{ $stats['petugas'] }}</h3>
        <p class="text-xs text-[#9a8b7a] mt-2">Pengelola sistem</p>
      </div>

      <div class="p-6 rounded-2xl bg-white shadow-lg">
        <p class="text-xs text-[#6b5a4a]">Total User</p>
        <h3 class="text-3xl font-semibold">{{ $stats['user'] }}</h3>
        <p class="text-xs text-[#9a8b7a] mt-2">Anggota aktif</p>
      </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
      ...
      <div class="p-6 bg-white rounded-2xl shadow-lg">
        ...
        <ul class="text-sm space-y-2">
          <li>Menunggu: <b>{{ $stats['pending'] }}</b></li>
          <li>Disetujui: <b>{{ $stats['approved'] }}</b></li>
          <li>Ditolak: <b>{{ $stats['rejected'] }}</b></li>
        </ul>
      </div>
      ...
      <div class="p-6 bg-white rounded-2xl shadow-lg">
        ...
        <ul class="text-sm space-y-2">
          <li>Dipinjam: <b>{{ $stats['dipinjam'] }}</b></li>
          <li>Proses Kembali: <b>{{ $stats['proses_kembali'] }}</b></li>
          <li>Dikembalikan: <b>{{ $stats['dikembalikan'] }}</b></li>
        </ul>
      </div>
      ...
      <div class="p-6 bg-white rounded-2xl shadow-lg">
        ...
        <ul class="text-sm space-y-2">
          <li>User Aktif: <b>{{ $stats['user'] }}</b></li>
          <li>Petugas Aktif: <b>{{ $stats['petugas'] }}</b></li>
        </ul>
      </div>
