...
<head>
    ...
    <title>Laporan Proses Pengembalian - Admin</title>
    ...
    <style>
        ...
        .kondisi-dipinjam { background: #dbeafe; color: #1e40af; border: 1px solid #bfdbfe; }
        .kondisi-terlambat { background: #fef3c7; color: #d97706; border: 1px solid #fcd34d; }
        .kondisi-menunggu { background: #ecfdf5; color: #047857; border: 1px solid #6ee7b7; }
        ...
        <main class="flex-1 bg-[#faf6ee]/80 rounded-3xl p-8 shadow-xl max-w-[1400px] mx-auto">
            ...
            <div class="header-section">
                <div class="flex items-center justify-between mb-6">
                    <div>
                        <h2 class="judul text-4xl font-bold mb-1">Laporan Proses Pengembalian</h2>
                        <p class="text-[#7a5c45] text-sm">Laporan peminjaman buku per tanggal {{ date('d M Y') }}</p>
                    </div>
                    <div class="flex gap-3">
                        <a href="{{ route('admin.manajemenbuku') }}"
                           class="inline-flex items-center gap-2 px-5 py-3 bg-[#6b7280] hover:bg-[#4b5563] text-white rounded-xl font-medium shadow-lg hover:scale-[1.03] transition">
                            <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-width="2" d="M15 19l-7-7 7-7"/>
                            </svg>
                            Kembali
                        </a>
                        <a href="{{ route('admin.laporan.proses_pengembalian_pdf') }}"
                ...
                <div class="stats-card bg-gradient-to-r from-[#e2d3c1] to-[#d4c4a8] rounded-xl p-6 mb-6 border border-[#c9a44c]/20">
                    <div class="flex items-center justify-between">
                        ...
                        <div class="text-right">
                            ...
                            <p class="text-2xl font-bold">Proses Pengembalian</p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            @else
                <section>
                    <h3 class="judul text-xl mb-4">Proses Pengembalian</h3>
                        ...
                        <tbody class="divide-y divide-gray-200">
                            @foreach($items as $index => $item)
                                <tr class="border-b hover:bg-[#faf6ee] transition">
                                    <td class="px-4 py-3 text-sm font-medium text-[#3e2a1f]">{{ $index + 1 }}</td>
