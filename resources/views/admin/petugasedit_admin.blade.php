...
<div class="min-h-screen flex gap-6 p-6">
  ...
  <main class="flex-1 bg-[#faf6ee]/90 rounded-3xl p-10 shadow-xl">
    ...
    <h2 class="judul text-3xl mb-2">Edit Petugas</h2>
  ...
  <input type="text" name="username" required autocomplete="off"
      ...
      <div>
        ...
        <input type="date" name="tanggal_dibuat"
               value="{{ old('tanggal_dibuat', $petugas->tanggal_dibuat) }}"
               class="w-full px-4 py-3 rounded-xl border border-[#e2d3c1] bg-[#f3eee6] text-[#7a5c45] shadow">
      </div>
        ...
        <div class="flex flex-col gap-3">
          ...
          <div id="frame" class="w-48 h-48 border-2 border-dashed border-[#c9a44c] rounded-xl overflow-hidden bg-[#f3eee6] relative">
            <img id="previewFoto" class="absolute top-0 left-0 cursor-grab"
                 src="{{ $petugas->foto_profil ? asset('storage/'.$petugas->foto_profil) : '' }}"
                 alt="Preview Foto"
                 @if(!$petugas->foto_profil) hidden @endif>
            <span id="placeholderIcon" class="absolute inset-0 flex items-center justify-center text-xs text-[#7a5c45]"
                  @if($petugas->foto_profil) style="display:none;" @endif>
              Preview
            </span>
          </div>
      ...
      <div>
        ...
        <select name="jenis_kelamin" required class="w-full px-4 py-3 rounded-xl border border-[#e2d3c1] shadow focus:ring-2 focus:ring-[#c9a44c] focus:outline-none">
          ...
          <option value="L" @if($petugas->jenis_kelamin == 'L') selected @endif>Laki-laki</option>
          <option value="P" @if($petugas->jenis_kelamin == 'P') selected @endif>Perempuan</option>
        </select>
      </div>
