...
<head>
    ...
    <title>Laporan Kategori Buku - Admin</title>
        ...
        <main class="flex-1 bg-[#faf6ee]/80 rounded-3xl p-8 shadow-xl max-w-[1400px] mx-auto">
            ...
            <div class="header-section">
                <div class="flex items-center justify-between mb-6">
                    <div>
                        <h2 class="judul text-4xl font-bold mb-1">Laporan Kategori Buku</h2>
                        <p class="text-[#7a5c45] text-sm">Laporan data kategori buku per tanggal {{ date('d M Y') }}</p>
                    </div>
                    <div class="flex gap-3">
                        ...
                        <a href="{{ route('laporankategori_pdf_admin') }}"
                ...
                <!-- STATS CARD -->
                <div class="stats-card bg-gradient-to-r from-[#e2d3c1] to-[#d4c4a8] rounded-xl p-6 mb-6 border border-[#c9a44c]/20">
                    <div class="flex items-center justify-between">
                        <div>
                            <p class="text-sm opacity-80 mb-1 font-medium">Total Data Ditemukan</p>
                            <p class="text-3xl font-bold">{{ $kategori->count() }}</p>
                        </div>
                        <div class="text-right">
                            <p class="text-sm opacity-80 mb-1">Jenis Laporan</p>
                            <p class="text-2xl font-bold">Data Kategori Buku</p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            @if($kategori->isEmpty())
                <div class="bg-white rounded-2xl shadow-lg overflow-hidden">
                    <div class="empty-state">
                        ...
                        <p class="empty-state-subtext">Tidak ada data kategori buku</p>
                    </div>
                </div>
            @else
                    ...
                    <table class="min-w-full bg-white">
                        <thead class="bg-gradient-to-r from-[#e7dcc8] to-[#d9cbbe]">
                            <tr>
                                ...
                                <th class="px-4 py-3 text-left text-xs text-[#6b5a4a] font-semibold uppercase tracking-wider">Nama Kategori</th>
                                <th class="px-4 py-3 text-left text-xs text-[#6b5a4a] font-semibold uppercase tracking-wider">Jumlah Buku</th>
                                <th class="px-4 py-3 text-left text-xs text-[#6b5a4a] font-semibold uppercase tracking-wider">Tanggal Dibuat</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-gray-200">
                            @foreach($kategori as $index => $item)
                                <tr class="border-b hover:bg-[#faf6ee]/60 transition-colors duration-200">
                                    ...
                                    <td class="px-4 py-3 text-sm font-medium text-[#3e2a1f]">{{ $item->nama_kategori ?? '-' }}</td>
                                    <td class="px-4 py-3 text-sm text-[#555]">{{ $item->buku_count ?? 0 }} buku</td>
                                    <td class="px-4 py-3 text-sm font-medium text-[#3e2a1f] whitespace-nowrap">{{ $item->created_at ? \Carbon\Carbon::parse($item->created_at)->format('d/m/Y') : '-' }}</td>
                                </tr>
                            @endforeach
                        </tbody>
                    </table>
