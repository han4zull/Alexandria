              ...
              <tr class="border-b hover:bg-[#faf6ee] transition">
                ...
                <td class="px-4 py-3 text-center space-x-1">
                  ...
                  @if($p->status === 'menunggu konfirmasi')
                    <form action="{{ route('admin.peminjaman.setujui', $p->id) }}" method="POST" class="inline">
                      @csrf
                    ...
                    <form action="{{ route('admin.peminjaman.tolak', $p->id) }}" method="POST" class="inline ml-2">
                      @csrf
          ...
          @forelse($proses as $p)
            <div class="bg-white rounded-lg shadow p-5 border-l-4 border-[#c9a44c]">
              ...
              <div class="flex gap-3 flex-wrap items-center">
                <form action="{{ route('admin.peminjaman.proseskembali', $p->id) }}" method="POST" class="flex items-center gap-3 proses-form" data-peminjaman-id="{{ $p->id }}">
                  @csrf
                ...
                <form action="{{ route('admin.pengembalian.selesai', $p->id) }}" method="POST" class="inline selesai-form" data-peminjaman-id="{{ $p->id }}">
                  @csrf
...
<script>
  ...
  function navigateToLaporan() {
    ...
    const routes = {
      'admin.laporan.peminjaman_aktif': '{{ route("admin.laporan.peminjaman_aktif") }}',
      'admin.laporan.proses_pengembalian': '{{ route("admin.laporan.proses_pengembalian") }}',
      'admin.laporan.selesai': '{{ route("admin.laporan.selesai") }}'
    };

    let routeName = 'admin.laporan.peminjaman_aktif'; // default
    if (activeTab?.id === 'tab-proses') routeName = 'admin.laporan.proses_pengembalian';
    if (activeTab?.id === 'tab-selesai') routeName = 'admin.laporan.selesai';
