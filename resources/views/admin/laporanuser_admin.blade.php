...
<head>
    ...
    <title>Laporan User - Admin</title>
    ...
    <style>
        ...
        .status-aktif { background: #e6f9ef; color: #065f46; border: 1px solid #c7f0db; }
        .status-nonaktif { background: #fff3f4; color: #7f1d1d; border: 1px solid #f7caca; }
    </style>
</head>
        ...
        <main class="flex-1 bg-[#faf6ee]/80 rounded-3xl p-8 shadow-xl max-w-[1400px] mx-auto">
            ...
            <div class="header-section">
                <div class="flex items-center justify-between mb-6">
                    <div>
                        <h2 class="judul text-4xl font-bold mb-1">Laporan User</h2>
                        <p class="text-[#7a5c45] text-sm">Laporan data user per tanggal {{ date('d M Y') }}</p>
                    </div>
                    <div class="flex gap-3">
                        ...
                        <a href="{{ route('admin.laporan.user_pdf') }}"
                ...
                <div class="stats-card bg-gradient-to-r from-[#e2d3c1] to-[#d4c4a8] rounded-xl p-6 mb-6 border border-[#c9a44c]/20">
                    <div class="flex items-center justify-between">
                        ...
                        <div class="text-right">
                            ...
                            <p class="text-2xl font-bold">Data User</p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            @if($items->isEmpty())
                <div class="bg-white rounded-2xl shadow-lg overflow-hidden">
                    <div class="empty-state">
                        ...
                        <p class="empty-state-subtext">Tidak ada data user</p>
                    </div>
                </div>
            @else
                        ...
                        <tbody class="divide-y divide-gray-200">
                            @foreach($items as $index => $item)
                                <tr class="border-b hover:bg-[#faf6ee]/60 transition-colors duration-200">
                                    ...
                                    <td class="px-4 py-3 text-sm font-medium text-[#3e2a1f]">{{ $item->username ?? '-' }}</td>
                                    <td class="px-4 py-3 text-sm text-[#555]">{{ $item->nama_lengkap ?? '-' }}</td>
                                    <td class="px-4 py-3 text-sm text-[#555]">{{ $item->email ?? '-' }}</td>
