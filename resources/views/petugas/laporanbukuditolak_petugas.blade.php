...
<head>
    ...
    <title>Laporan Buku Ditolak - Petugas</title>
        ...
        <main class="flex-1 bg-[#faf6ee]/80 rounded-3xl p-8 shadow-xl max-w-[1400px] mx-auto">
            ...
            <div class="header-section">
                <div class="flex items-center justify-between mb-6">
                    <div>
                        <h2 class="judul text-4xl font-bold mb-1">Laporan Buku Ditolak</h2>
                        <p class="text-[#7a5c45] text-sm">Laporan buku yang telah ditolak per tanggal {{ date('d M Y') }}</p>
                    </div>
                    <div class="flex gap-3">
                        ...
                        <a href="{{ route('petugas.laporan.buku_ditolak_pdf') }}"
                           class="btn-action inline-flex items-center gap-2 px-5 py-3 bg-gradient-to-br from-[#c9a44c] to-[#8a6a3f] hover:from-[#d4af55] hover:to-[#936d42] text-white rounded-xl font-medium shadow-lg">
                            <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-width="1.6" d="M12 3v12m0 0l4-4m-4 4l-4-4M4 21h16"/>
                            </svg>
                            Unduh PDF
                        </a>
                    </div>
                ...
                <div class="stats-card bg-gradient-to-r from-[#e2d3c1]/30 to-[#7a5c45]/20 rounded-xl p-6 mb-6">
                    <div class="flex items-center justify-between">
                        <div>
                            <p class="text-sm opacity-80 mb-1 font-medium">Total Buku Ditolak</p>
                            <p class="text-3xl font-bold">{{ $buku->count() }}</p>
                        </div>
                        <div class="text-right">
                            <p class="text-sm opacity-80 mb-1">Status</p>
                            <p class="text-2xl font-bold">Buku Ditolak</p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            @if($buku->isEmpty())
                <div class="bg-white rounded-2xl shadow-lg overflow-hidden">
                    <div class="empty-state">
                        ...
                        <p class="empty-state-subtext">Tidak ada buku yang telah ditolak</p>
                    </div>
                </div>
            @else
                        ...
                        <tbody class="divide-y divide-gray-200">
                            ...
                            <tr class="hover:bg-[#faf6ee] transition-colors duration-200">
                                ...
                                <td class="px-4 py-3 text-sm">
                                    <span class="kondisi-badge kondisi-hilang">Ditolak</span>
                                </td>
                            </tr>
                            @endforeach
                        </tbody>
